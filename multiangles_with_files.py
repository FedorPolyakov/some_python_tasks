...
rects = []
for d in data:
    rects.append(d.strip('\n'))

for i in range(len(rects)):
    rects[i] = [tuple(map(int, i.replace(')','').replace('(','').split(','))) for i in rects[i].split(')(')]
...
def main(m1, m2):
    if check_list(m1) and check_list(m2):
        if len(m1) == len(m2):
            '''
            если координаты точек оказались не равны друг другу,
            то развернем один из списков в обратном направлении.
            проверим, может точки были введены для одного многоугольника по часовой стрелки,
            а для другого - против часовой.
            '''
...
with open('output.txt', 'w', encoding='utf-8') as file:
    file.write(main(rects[0], rects[1]))